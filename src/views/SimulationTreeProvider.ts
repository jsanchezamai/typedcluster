...
class SimulationTreeItem extends vscode.TreeItem {
    constructor(public simulation: Simulation) {
        super(simulation.name, vscode.TreeItemCollapsibleState.Collapsed);
 ...
 * ```
 */
export class SimulationTreeProvider
    implements vscode.TreeDataProvider<SimulationTreeItem>
{
    private _onDidChangeTreeData: vscode.EventEmitter<
        SimulationTreeItem | undefined | null
    > = new vscode.EventEmitter<SimulationTreeItem | undefined | null>();
     ...
     */
    async getChildren(
        element?: SimulationTreeItem
    ): Promise<SimulationTreeItem[]> {
        if (!element) {
            // Root level - show all simulations
            const simulations = this.simulationManager.getSimulations();
            const simtree = simulations.map(
                (sim) => new SimulationTreeItem(sim)
            );
            return simtree;
        }
        ...
        const details: SimulationTreeItem[] = [
            new SimulationDetailItem(
                `Nodes: ${element.simulation.nodeCount}`,
                element.simulation
            ),
            new SimulationDetailItem(
                `Type: ${element.simulation.workType}`,
                element.simulation
            ),
            new SimulationDetailItem(
                `Strategy: ${element.simulation.strategy}`,
                element.simulation
            ),
            new SimulationDetailItem(
                `Algorithm: ${element.simulation.algorithm}`,
                element.simulation
            ),
            new SimulationActionItem(
